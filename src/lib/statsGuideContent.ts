...
export interface StatGuideEntry {
  ...
  long: string[]
  /** Page body: optional labelled thresholds / bands (front-weight zones, ratio bands…). */
  bullets?: { label?: string; text: string }[]
  /** Page body: the bar↔sim "precise counterpart" pairing, or a key cross-reference. */
  seeAlso?: string
}
...
export const STAT_GUIDE_ENTRIES: StatGuideEntry[] = [
  ...
  // ─── Identity layer ────────────────────────────────────────────────────────
  {
    ...
    long: [
      "The performance bracket a car sits in, from D up through C, B, A, S1, S2 to R. Every race caps entries by class.",
      "Because racing is class-capped, a raw stat means little on its own — always read it against the class. A 150 mph top speed is poor for an A-class circuit car and excellent for a D-class hatch.",
    ],
  },
  {
    id: 'pi-rating',
  ...
  // ─── The six bar stats (0–10) ──────────────────────────────────────────────
  {
    ...
    seeAlso: "Precise counterpart: Top speed (simTopSpeed). Trust the sim number when they disagree.",
  },
  {
    id: 'handling',
    label: 'Handling',
    ...
    seeAlso: "Precise counterpart: Lateral G (simLateralG60/120), plus mech/aero balance for how that grip is distributed.",
  },
  {
    id: 'acceleration',
    label: 'Acceleration',
    ...
    seeAlso: "Precise counterpart: 0–60 and 0–100 (simZeroToSixty / simZeroToHundred), read together for acceleration shape.",
  },
  {
    id: 'launch',
    label: 'Launch',
    ...
    seeAlso: "Precise counterpart: the first slice of 0–60 (simZeroToSixty).",
  },
  {
    id: 'braking',
    label: 'Braking',
    ...
    short: "How hard the car can decelerate. High braking lets you brake later into hard zones — and the current meta rewards brake upgrades heavily.",
    long: [
      "How hard the car can slow down.",
      "High braking = brake later and carry confidence into hard braking zones. The current FH6 meta rewards brake upgrades heavily.",
    ],
    seeAlso: "Precise counterpart: 60–0 / 100–0 braking distance (simBraking60 / simBraking100) — feet to stop, lower is better.",
  },
  {
    id: 'offroad',
    label: 'Offroad',
    ...
    seeAlso: "No sim counterpart — this quality only lives as a bar stat.",
  },
  ...
  // ─── Raw specs ─────────────────────────────────────────────────────────────
  {
    ...
    seeAlso: "Read alongside Weight via Power-to-weight — the single most useful build-priority number.",
  },
  {
    id: 'torque',
    label: 'Torque',
  ...
  },
  {
    ...
    seeAlso: "Feeds Front weight, which in turn sets the Mechanical balance number — one weight figure predicts braking, balance, and where to spend PI.",
  },
  {
    id: 'front-weight',
    label: 'Front weight',
    ...
    bullets: [
      { label: '> 55% (nose-heavy)', text: "Understeer tendency, FWD-like — stronger front braking grip but lazy rotation." },
      { label: '< 45% (tail-heavy)', text: "Oversteer / rotation-happy (rear- and mid-engine cars); twitchy on lift-off." },
      { label: '~ 50% (neutral)',    text: "The tuner's friend — a balanced starting point." },
    ],
    seeAlso: "Inversely correlated with Mechanical balance (r = −0.61): a nose-heavy car reads a low mech-balance number.",
  },
  {
    id: 'displacement',
    label: 'Displacement',
  ...
  },
  {
    ...
    bullets: [
      { label: '≤ 6 lb/hp',   text: "Already power-rich — spend PI on grip (tyres, suspension, aero)." },
      { label: '8–12 lb/hp',   text: "Balanced — build to the race type." },
      { label: '≥ 13 lb/hp',  text: "Heavy for its power — straight-line pace is the limiter; spend PI on power or weight first." },
    ],
  },
  ...
  // ─── Simulation metrics ────────────────────────────────────────────────────
  {
    ...
    seeAlso: "Precise counterpart to the Launch and Acceleration bars.",
  },
  {
    id: 'zero-to-hundred',
    label: '0–100 mph',
    ...
    seeAlso: "Acceleration shape = 0–100 ÷ 0–60. ≤ ~2.1 keeps pulling up top (long circuits); ≥ ~2.6 is front-loaded (street starts, short sprints).",
  },
  {
    id: 'top-speed',
    label: 'Top speed',
    ...
    seeAlso: "Precise counterpart to the Speed bar. Trust this figure when the bar disagrees.",
  },
  {
    id: 'braking-distance',
    ...
    seeAlso: "Precise counterpart to the Braking bar. Read against class — see the Appendix per-class braking baselines.",
  },
  {
    id: 'lateral-g',
    label: 'Lateral G',
    ...
    bullets: [
      { label: 'Ratio ≥ ~1.12 (top ~20%)', text: "Downforce-driven — comes alive at speed, strong in fast sweepers, can feel inert when slow." },
      { label: 'Ratio ≤ ~1.0',             text: "Little aero benefit — a low-speed, point-and-squirt chassis, better on tight layouts." },
    ],
    seeAlso: "Precise counterpart to the Handling bar (with mech/aero balance). Grip rising with speed is normal here (median ratio ≈ 1.03); a falling ratio essentially never happens.",
  },
  {
    id: 'aero-efficiency',
  ...
  },
  {
    ...
    bullets: [
      { label: '≤ ~0.43', text: "Front-grip-biased → understeer at low speed → soften front ARB / stiffen rear." },
      { label: '~0.50',      text: "Neutral." },
      { label: '≥ ~0.61', text: "Rear-grip-biased → oversteer at low speed → stiffen front ARB / soften rear." },
    ],
    seeAlso: "The low-speed partner to Aero balance (the high-speed lever). Inversely tied to Front weight.",
  },
  {
    id: 'aero-balance',
    label: 'Aero balance',
    ...
    seeAlso: "The direction here is the least battle-tested field — sanity-check against in-game feel before leaning on it.",
  },
]
